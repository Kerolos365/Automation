...
public class ManageUsersPage extends BaseTest {
    ...
    @Test(priority = 2, description = "Check export button")
    ...
    public void SearchUsers() throws IOException, InterruptedException {
        ...
        for (WebElement org : Users) {
            if (org.getText().trim().contains("Test")) {
                isFound = true;
                break;
            }
        }
        Assert.assertTrue(isFound, "Search user '" + "Test" + "' was not found in the users list");
    }
    ...
    public void ClickAddUserButton() throws IOException, InterruptedException {
        ...
        Thread.sleep(2000); // Wait for the page to load
        // 10. Verify that the new user is added to the list
        boolean isFound = false;
        while (true) {
            List<WebElement> users = driver.findElements(UsersColumnBy);
    ...
    public void ClickAddUserButtonInvalidData() throws IOException, InterruptedException {
        ...
        Assert.assertTrue(usernameValidationMsgElement.isDisplayed(), "Username validation message is not displayed");
          // 7.3 Verify that the Confirm password match error message is displayed
    ...
    @Test(priority = 9, description = "Edit super User")
    public void EditSuperUserUsers() throws IOException, InterruptedException {
        ...
        // 2. Wait for the overlay to disappear
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        wait.until(ExpectedConditions.invisibilityOfElementLocated(By.className("MuiBackdrop-root")));

        // 3. click the User Action button
        WebElement UserActionButton = wait.until(ExpectedConditions.elementToBeClickable(UserActionButtonBy));
        ...
        // 5. Verify that the system redirects to the Edit User page
        String currentUrl = driver.getCurrentUrl();
        ...
        // 7. Change the Address for the user
        ...
        UserAddressField.sendKeys(Keys.CONTROL + "a");
        UserAddressField.sendKeys(Keys.DELETE);
        ...
        // 8. Scroll the Update button into view to ensure it's clickable
        ...
        // 10. Verify that the system not redirects to the manage User page and show  Not Authorized
        WebElement UserUpdateMessage = driver.findElement(UserUpdateMessageBy);
        String messageText = UserUpdateMessage.getText();
        Assert.assertTrue(messageText.contains("User updated successfully"),
                "Expected to contain 'User updated successfully' but got: " + messageText);
    ...
    public void EditNothingCUsers() throws IOException, InterruptedException {
        ...
        // 7. Click on update button without changing anything
        WebElement UserUpdateButton = driver.findElement(UserUpdateButtonBy);
        wait.until(ExpectedConditions.elementToBeClickable(UserUpdateButton)).click();
    ...
    @Test(priority = 11, description = "View Users")
    public void ViewUsers() throws IOException, InterruptedException {
        ...
        // 5. Verify that the system redirects to the View User page
